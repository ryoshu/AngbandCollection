# Building NuAngband on Modern macOS

This document describes how to build NuAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `nuangband-terminal` in the parent directory.
...
### Available Targets
- `nuangband` - Build executable only
...
After building:
```bash
# From the main NuAngband directory:
./nuangband-terminal
```
...
- Make sure you're running from the NuAngband directory (where lib/ is)
