# Building GilAngband on Modern macOS

This document describes how to build GilAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `gilangband-terminal` in the parent directory.
...
### Available Targets
- `gilangband` - Build executable only
...
After building:
```bash
# From the main GilAngband directory:
./gilangband-terminal
```
...
- Make sure you're running from the GilAngband directory (where lib/ is)
