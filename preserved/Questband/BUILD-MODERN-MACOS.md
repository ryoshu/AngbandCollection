# Building Questband on Modern macOS

This document describes how to build Questband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `questband-terminal` in the parent directory.
...
### Available Targets
- `questband` - Build executable only
...
After building:
```bash
# From the main Questband directory:
./questband-terminal
```
...
- Make sure you're running from the Questband directory (where lib/ is)
