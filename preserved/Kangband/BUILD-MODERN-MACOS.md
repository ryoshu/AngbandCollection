# Building Kangband on Modern macOS

This document describes how to build Kangband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `kangband-terminal` in the parent directory.
...
### Available Targets
- `kangband` - Build executable only
...
After building:
```bash
# From the main Kangband directory:
./kangband-terminal
```
...
- Make sure you're running from the Kangband directory (where lib/ is)
