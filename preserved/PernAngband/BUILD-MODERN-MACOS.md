# Building PernAngband on Modern macOS

This document describes how to build PernAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `pernangband-terminal` in the parent directory.
...
### Available Targets
- `pernangband` - Build executable only
...
After building:
```bash
# From the main PernAngband directory:
./pernangband-terminal
```
...
- Make sure you're running from the PernAngband directory (where lib/ is)
