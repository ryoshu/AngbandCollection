# Building Conglomoband on Modern macOS

This document describes how to build Conglomoband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `conglomoband-terminal` in the parent directory.
...
### Available Targets
- `conglomoband` - Build executable only
...
After building:
```bash
# From the main Conglomoband directory:
./conglomoband-terminal
```
...
- Make sure you're running from the Conglomoband directory (where lib/ is)
