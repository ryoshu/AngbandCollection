# Building Diabloband on Modern macOS

This document describes how to build Diabloband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `diabloband-terminal` in the parent directory.
...
### Available Targets
- `diabloband` - Build executable only
...
After building:
```bash
# From the main Diabloband directory:
./diabloband-terminal
```
...
- Make sure you're running from the Diabloband directory (where lib/ is)
