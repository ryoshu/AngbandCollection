# Building IsoAngband on Modern macOS

This document describes how to build IsoAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `isoangband-terminal` in the parent directory.
...
### Available Targets
- `isoangband` - Build executable only
...
After building:
```bash
# From the main IsoAngband directory:
./isoangband-terminal
```
...
- Make sure you're running from the IsoAngband directory (where lib/ is)
