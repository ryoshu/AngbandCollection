# Building Multiband on Modern macOS

This document describes how to build Multiband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `multiband-terminal` in the parent directory.
...
### Available Targets
- `multiband` - Build executable only
...
After building:
```bash
# From the main Multiband directory:
./multiband-terminal
```
...
- Make sure you're running from the Multiband directory (where lib/ is)
