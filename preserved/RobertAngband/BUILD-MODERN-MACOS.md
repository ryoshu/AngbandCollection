# Building RobertAngband on Modern macOS

This document describes how to build RobertAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `robertangband-terminal` in the parent directory.
...
### Available Targets
- `robertangband` - Build executable only
...
After building:
```bash
# From the main RobertAngband directory:
./robertangband-terminal
```
...
- Make sure you're running from the RobertAngband directory (where lib/ is)
