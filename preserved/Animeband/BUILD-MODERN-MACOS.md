# Building Animeband on Modern macOS

This document describes how to build Animeband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `animeband-terminal` in the parent directory.
...
### Available Targets
- `animeband` - Build executable only
...
After building:
```bash
# From the main Animeband directory:
./animeband-terminal
```
...
- Make sure you're running from the Animeband directory (where lib/ is)
