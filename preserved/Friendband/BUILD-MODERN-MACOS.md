# Building Friendband on Modern macOS

This document describes how to build Friendband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `friendband-terminal` in the parent directory.
...
### Available Targets
- `friendband` - Build executable only
...
After building:
```bash
# From the main Friendband directory:
./friendband-terminal
```
...
- Make sure you're running from the Friendband directory (where lib/ is)
