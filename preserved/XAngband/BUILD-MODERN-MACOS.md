# Building XAngband on Modern macOS

This document describes how to build XAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `xangband-terminal` in the parent directory.
...
### Available Targets
- `xangband` - Build executable only
...
After building:
```bash
# From the main XAngband directory:
./xangband-terminal
```
...
- Make sure you're running from the XAngband directory (where lib/ is)
