# Building Jackband on Modern macOS

This document describes how to build Jackband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `jackband-terminal` in the parent directory.
...
### Available Targets
- `jackband` - Build executable only
...
After building:
```bash
# From the main Jackband directory:
./jackband-terminal
```
...
- Make sure you're running from the Jackband directory (where lib/ is)
