# Building PsiAngband on Modern macOS

This document describes how to build PsiAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `psiangband-terminal` in the parent directory.
...
### Available Targets
- `psiangband` - Build executable only
...
After building:
```bash
# From the main PsiAngband directory:
./psiangband-terminal
```
...
- Make sure you're running from the PsiAngband directory (where lib/ is)
