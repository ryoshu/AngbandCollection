# Building TeamAngband on Modern macOS

This document describes how to build TeamAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `teamangband-terminal` in the parent directory.
...
### Available Targets
- `teamangband` - Build executable only
...
After building:
```bash
# From the main TeamAngband directory:
./teamangband-terminal
```
...
- Make sure you're running from the TeamAngband directory (where lib/ is)
