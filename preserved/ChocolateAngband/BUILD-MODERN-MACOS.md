# Building ChocolateAngband on Modern macOS

This document describes how to build ChocolateAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `chocolateangband-terminal` in the parent directory.
...
### Available Targets
- `chocolateangband` - Build executable only
...
After building:
```bash
# From the main ChocolateAngband directory:
./chocolateangband-terminal
```
...
- Make sure you're running from the ChocolateAngband directory (where lib/ is)
