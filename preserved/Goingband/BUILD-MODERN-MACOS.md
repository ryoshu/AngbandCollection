# Building Goingband on Modern macOS

This document describes how to build Goingband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `goingband-terminal` in the parent directory.
...
### Available Targets
- `goingband` - Build executable only
...
After building:
```bash
# From the main Goingband directory:
./goingband-terminal
```
...
- Make sure you're running from the Goingband directory (where lib/ is)
