# Building Jackalband on Modern macOS

This document describes how to build Jackalband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `jackalband-terminal` in the parent directory.
...
### Available Targets
- `jackalband` - Build executable only
...
After building:
```bash
# From the main Jackalband directory:
./jackalband-terminal
```
...
- Make sure you're running from the Jackalband directory (where lib/ is)
