# Building DaJAngband on Modern macOS

This document describes how to build DaJAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `dajangband-terminal` in the parent directory.
...
### Available Targets
- `dajangband` - Build executable only
...
After building:
```bash
# From the main DaJAngband directory:
./dajangband-terminal
```
...
- Make sure you're running from the DaJAngband directory (where lib/ is)
