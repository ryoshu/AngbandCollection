# Building Entroband on Modern macOS

This document describes how to build Entroband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `entroband-terminal` in the parent directory.
...
### Available Targets
- `entroband` - Build executable only
...
After building:
```bash
# From the main Entroband directory:
./entroband-terminal
```
...
- Make sure you're running from the Entroband directory (where lib/ is)
