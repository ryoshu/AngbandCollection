# Building EricAngband on Modern macOS

This document describes how to build EricAngband on modern macOS systems (macOS 10.15+, including Apple Silicon Macs).
...
This creates `ericangband-terminal` in the parent directory.
...
### Available Targets
- `ericangband` - Build executable only
...
After building:
```bash
# From the main EricAngband directory:
./ericangband-terminal
```
...
- Make sure you're running from the EricAngband directory (where lib/ is)
